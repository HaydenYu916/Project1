...
def parse_pair_list(text: str) -> list[tuple[int, int]]:
    ...
    Each pair is separated by ';' (or whitespace). Within a pair, R and B
    are separated by ','. Both values must be in [0, 100]. Returns
    [(R, B), ...] with the order preserved.
    """
    pairs: list[tuple[int, int]] = []
    # Allow ';' as the canonical pair separator; also accept newlines/spaces
    # for ergonomics when a long pair list is provided via @file or env.
    for sep in ("\n", "\t"):
    ...
    for item in text.split(";"):
        ...
        if r == 0 and b == 0:
            # Skip pure-dark segments — same convention as ratios (0:0 banned).
...
def create_segment_conditions(
    ...
    if pairs:
        # (pwm_r, pwm_b, total_pwm, ratio_r_col, ratio_b_col)
