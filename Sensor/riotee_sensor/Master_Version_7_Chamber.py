...
def setup_csv():
    """
    [CSV记录] 设置CSV文件
    功能: 创建带时间戳的CSV文件并初始化写入器
    防覆写机制: 自动检查文件存在，添加序号避免覆写
    """
    ...
    if args.csv_name:
        ...
        if args.no_timestamp:
            # 不使用时间戳，直接使用指定名称
            filename = f"{args.csv_name}.csv"
            comment = args.comment if args.comment is not None else args.csv_name
        else:
            # 在指定名称后添加时间戳
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            filename = f"{args.csv_name}_{timestamp}.csv"
            comment = args.comment if args.comment is not None else f"{args.csv_name}_{timestamp}"
    else:
        # 自动生成带时间戳的文件名
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = f"riotee_data_{timestamp}.csv"
        comment = args.comment if args.comment is not None else f"riotee_data_{timestamp}"
    
    # 生成唯一文件路径
    filepath = generate_unique_filename(filename)
    
    # 创建CSV文件 - 新增增益和休眠时间字段
    fieldnames = ['id', 'timestamp', 'device_id', 'temperature', 'humidity', 
                  'a1_raw', 'vcap_raw'] + [f'sp_{i}' for i in [415, 445, 480, 515, 555, 590, 630, 680]] + ['sp_clear', 'sp_nir', 'spectral_gain', 'sleep_time']
    
    csv_file = open(filepath, 'w', newline='')
    csv_writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
    
    # 写入注释行
    csv_file.write(f"# {comment}\n")
    csv_writer.writeheader()
    
    logging.info(f"CSV文件创建: {filepath}")
    return filepath
...
def process_device_data(device_id, data_arr):
    """
    [数据处理] 处理设备数据
    功能: 解析Riotee原始数据，同时保存到CSV和发布到MQTT
    包含: 数据解析 + CSV记录 + MQTT上传
    支持: 二进制数据格式和JSON格式
    新增: 光谱传感器增益值和设备休眠时间字段
    """
    global record_id, csv_writer
    ...
    row.update({k: f"{v:.2f}" for k, v in spectrum_data.items()})
...
def main():
    ...
    if args.csv_name:
        ...
        logging.info(f"  - 时间戳命名: {'关闭' if args.no_timestamp else '开启'}")
    ...
    # [初始化] CSV记录模块
    csv_path = setup_csv()
    logging.info(f"数据将保存到: {csv_path}")
                ...
                for dev_id in devices:
                    ...
                    for pkt in packets:
                        try:
                            ...
                            process_device_data(dev_id, data_arr)
