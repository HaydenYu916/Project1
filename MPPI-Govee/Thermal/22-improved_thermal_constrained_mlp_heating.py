...
class ImprovedThermodynamicConstrainedMLPHeating:
    """改进版：基于热力学约束的MLP模型 - 开灯阶段"""
    ...
    @staticmethod
    def thermal_heating_response(t, a1_val, K1_base, tau1, K2_base, tau2, alpha_solar):
        """
        热力学升温响应函数
        ΔT(t) = K1(a1) × (1 - exp(-t/τ1)) + K2(a1) × (1 - exp(-t/τ2))
        """
        a1_ref = 1.4
    ...
    def fit_thermal_model_improved(self, t_data, a1_data, temp_diff_data):
        ...
        try:
            ...
            def thermal_wrapper(t, K1_base, tau1, K2_base, tau2, alpha_solar):
                return self.thermal_heating_response(t, a1_data, K1_base, tau1, K2_base, tau2, alpha_solar)
            ...
            tau1_guess = 20  # 更合理的快速升温时间
            tau2_guess = 150  # 更合理的慢速升温时间
    ...
    def fit(self, t_data, a1_data, temp_diff_data):
        ...
        try:
            ...
            print(f"     改进热力学升温模型性能: R²={thermal_r2:.3f}, RMSE={thermal_rmse:.3f}")
    ...
    def predict_thermal_only(self, t_data, a1_data):
        ...
        return self.thermal_heating_response(
            t_data, a1_data,
            self.thermal_params['K1_base'],
            self.thermal_params['tau1'],
            self.thermal_params['K2_base'],
...
def prepare_averaged_heating_data(csv_files):
    """准备按PPFD分组的平均开灯数据"""
    print("📊 准备按PPFD分组的平均开灯数据...")
    ...
    for csv_file in csv_files:
        ...
        # 只使用开灯数据
        led_on_data = df[df['led_status'] == 1].copy()
        
        if len(led_on_data) > 10:
            # 计算从开灯开始的时间
            led_on_data['time_from_start'] = (led_on_data['timestamp'] - led_on_data['timestamp'].iloc[0]).dt.total_seconds() / 60
            
            # 添加数据
            for _, row in led_on_data.iterrows():
                all_data.append({
...
def train_improved_thermal_constrained_mlp_heating(data):
    """训练改进的热力学约束MLP模型（开灯阶段）"""
    print("\n🔬 训练改进的热力学约束MLP模型（开灯阶段）...")
    ...
    # 创建并训练改进的热力学约束MLP模型
    model = ImprovedThermodynamicConstrainedMLPHeating()
    success, r2, rmse, pred = model.fit(t_data, a1_data, temp_diff_data)
    
    if success:
        print(f"   改进的热力学约束MLP模型性能（开灯阶段）: R²={r2:.3f}, RMSE={rmse:.3f}")
        return model
    else:
...
def generate_improved_heating_predictions(model, solar_values):
    """生成改进的开灯阶段预测"""
    print(f"\n🔮 生成改进的开灯阶段热力学约束MLP预测...")
    
    predictions = {}
    time_points = np.linspace(0, 800, 200)  # 开灯阶段时间较长
    
    for solar_val in solar_values:
        print(f"   生成Solar {solar_val:.3f}的开灯预测...")
...
def plot_improved_heating_comparison(predictions, original_data, output_dir):
    """绘制改进的开灯阶段对比图"""
    print(f"\n🎨 绘制改进的开灯阶段对比图...")
    
    # 创建图形
    fig, axes = plt.subplots(2, 2, figsize=(20, 16))
    fig.suptitle('改进的热力学约束MLP模型 vs 纯热力学模型对比（开灯阶段）', fontsize=16, fontweight='bold')
    ...
    # 左上：热力学约束MLP预测
    ax1 = axes[0, 0]
    ax1.set_title('改进的热力学约束MLP预测（开灯）', fontsize=14)
    ...
    # 右上：纯热力学模型预测
    ax2 = axes[0, 1]
    ax2.set_title('改进的纯热力学模型预测（开灯）', fontsize=14)
    ...
    # 左下：Solar 1.296详细对比
    ax3 = axes[1, 0]
    ax3.set_title('Solar 1.296 开灯详细对比', fontsize=14)
    ...
    # 右下：Solar 1.438详细对比
    ax4 = axes[1, 1]
    ax4.set_title('Solar 1.438 开灯详细对比', fontsize=14)
    ...
    # 保存图片
    output_file = output_dir / "Improved_Thermal_Constrained_MLP_Heating_Comparison.png"
    plt.savefig(output_file, dpi=200, bbox_inches='tight')
    print(f"   📊 保存改进开灯对比图: {output_file}")
    plt.close()


def save_improved_heating_results(model, predictions, original_data, output_dir):
    """保存改进的开灯阶段结果"""
    results = {
        'model_type': 'Improved Thermodynamic Constrained MLP Model - Heating Phase',
    ...
    for solar_val, pred_data in predictions.items():
        results['predictions_summary'][str(solar_val)] = {
            ...
            'thermal_constrained_max': float(np.max(pred_data['thermal_constrained_pred'])),
            'thermal_only_max': float(np.max(pred_data['thermal_only_pred'])),
            'improvement': float(np.max(pred_data['thermal_constrained_pred']) - np.max(pred_data['thermal_only_pred']))
        }
    ...
    # 保存JSON结果
    output_file = output_dir / "Improved_Thermal_Constrained_MLP_Heating_Results.json"
    
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    
    print(f"   💾 保存改进开灯阶段结果: {output_file}")


def main():
    """主函数"""
    print("🔬 改进的热力学约束MLP模型分析（开灯阶段）")
    print("=" * 60)
    ...
    # 准备按PPFD分组的平均开灯数据
    data = prepare_averaged_heating_data(csv_files)
    
    if len(data) == 0:
        print("❌ 没有有效的开灯数据")
        return
    
    print(f"📊 准备了 {len(data)} 个平均开灯数据点")
    
    # 训练改进的热力学约束MLP模型（开灯阶段）
    model = train_improved_thermal_constrained_mlp_heating(data)
    ...
    predictions = generate_improved_heating_predictions(model, solar_values)
    ...
    # 绘制对比图
    plot_improved_heating_comparison(predictions, data, plot_dir)
    
    # 保存结果
    save_improved_heating_results(model, predictions, data, result_dir)
    ...
    # 保存MLP模型
    mlp_model_path = export_dir / "heating_mlp_model.pkl"
    with open(mlp_model_path, 'wb') as f:
        pickle.dump(model, f)
    print(f"   💾 保存MLP模型: {mlp_model_path}")
    
    # 保存热力学模型参数
    thermal_model_path = export_dir / "heating_thermal_model.json"
    thermal_model_data = {
    ...
    print(f"\n🎉 改进的热力学约束MLP模型分析完成（开灯阶段）!")
    print(f"   训练了改进的热力学约束MLP模型（开灯阶段）")
    print(f"   生成了 {len(predictions)} 个Solar值的开灯预测曲线")
