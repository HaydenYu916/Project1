...
class MPPIControlV2:
    ...
    def _send_pwm(self, r_pwm: float, b_pwm: float) -> Dict[str, Any]:
        ...
        if not self.live:
            # ⚠️ Dry-run:不真实下发,只回报"会发什么"
            self._log_simple(
                f"[dry-run] 拟下发 PWM: 红 {brightness_r}% / 蓝 {brightness_b}% (未真实推送)"
            )
            result["red"]  = {"success": True, "verified": False, "retries": 0, "response": None, "error": "dry-run"}
            result["blue"] = {"success": True, "verified": False, "retries": 0, "response": None, "error": "dry-run"}
            return result
        ...
        if "Red" in self.devices:
            ...
            if result["red"]["success"]:
                ...
                self._log_simple(f"❌ 红光设备设置失败: {result['red'].get('error', '未知错误')}")
        ...
        if "Blue" in self.devices:
            ...
            if result["blue"]["success"]:
                ...
                self._log_simple(f"❌ 蓝光设备设置失败: {result['blue'].get('error', '未知错误')}")

        if not self.background:
            print(f"📊 发送完成 - 红: {result['red']}, 蓝: {result['blue']}")
