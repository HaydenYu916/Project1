...
from sklearn.model_selection import (
    GroupKFold, GroupShuffleSplit, KFold, RandomizedSearchCV, train_test_split,
)
...
def main():
    ...
    # Group rows by (x_cm, y_cm, z_cm). Splits and inner CV use this so a
    # position never appears in both train and val/test — that's how we
    # measure cross-position generalization rather than rewarding the
    # interpolator that memorized each training position.
    pos_cols = [c for c in ("x_cm", "y_cm", "z_cm") if c in df_model.columns]
    if not pos_cols:
        raise ValueError("group-cv variant requires x_cm/y_cm/z_cm columns in the CSV")
    groups_full = df_model[pos_cols].astype(str).agg("|".join, axis=1).values
    n_groups = len(set(groups_full))
    print(f"  groups (positions): {n_groups} unique")

    gss_test = GroupShuffleSplit(n_splits=1, test_size=test_ratio,
                                 random_state=args.random_state)
    trainval_idx, test_idx = next(gss_test.split(X_full, y_full, groups_full))
    X_trainval = X_full.iloc[trainval_idx].reset_index(drop=True)
    y_trainval = y_full.iloc[trainval_idx].reset_index(drop=True)
    X_test = X_full.iloc[test_idx].reset_index(drop=True)
    y_test = y_full.iloc[test_idx].reset_index(drop=True)
    groups_trainval = groups_full[trainval_idx]

    gss_val = GroupShuffleSplit(n_splits=1, test_size=val_ratio_within_trainval,
                                random_state=args.random_state)
    ...
    train_pos = sorted(set(groups_train))
    val_pos = sorted(set(groups_trainval[val_idx]))
    test_pos = sorted(set(groups_full[test_idx]))
    print(f"  train positions ({len(train_pos)}): {train_pos}")
    print(f"  val   positions ({len(val_pos)}): {val_pos}")
    print(f"  test  positions ({len(test_pos)}): {test_pos}")
    ...
    if not args.skip_tuning:
        ...
        tuned_rows = []
        # GroupKFold caps n_splits at unique group count
        unique_train_groups = len(set(groups_train))
        inner_cv_splits = min(args.cv_folds, max(2, unique_train_groups))
        if inner_cv_splits != args.cv_folds:
            print(f"  ⚠️ cv_folds {args.cv_folds} > unique train positions {unique_train_groups}; "
                  f"using {inner_cv_splits} GroupKFolds instead")
