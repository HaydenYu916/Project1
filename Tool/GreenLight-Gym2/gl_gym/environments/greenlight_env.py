...
from gl_gym.models.GreenLight.utils import define_model, define_aux_outputs
...
class GreenLightEnv(gym.Env):
    def __init__(
        ...
        # auxiliary CasADi function to read off canopy photosynthesis fluxes
        # at the integrated state. Cheap (single symbolic eval), no integration.
        self.F_aux = define_aux_outputs(
            nx=self.nx,
            nu=self.nu,
            nd=self.nd,
            n_params=self.num_params,
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, SupportsFloat, bool, bool, Dict[str, Any]]:
        ...
        try:
            d_now = self.weather_data[self.timestep]
            p_dyn = ca.vertcat(ca.DM(d_now), self.p)
            res = self.F(x0=ca.DM(self.x), u=ca.DM(self.u), p=p_dyn)
            self.x = res["xf"].full().flatten()

            # evaluate canopy photosynthesis fluxes at the integrated state
            aux = self.F_aux(
                x=ca.DM(self.x),
                u=ca.DM(self.u),
                d=ca.DM(d_now),
    ...
    def _get_info(self, reward_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if hasattr(self, "aux_outputs"):
            info.update(self.aux_outputs)
        return info
