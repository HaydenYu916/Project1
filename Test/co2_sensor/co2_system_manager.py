...
class CO2System:
    def __init__(self):
        ...
        self.collector_script = self.base_dir / "co2_collector.py"
        self.logs_path = self.base_dir / "logs"  # 使用本地logs目录
        self.pid_file = self.base_dir / "co2_collector.pid"
        self.log_file = self.base_dir / "co2_system_manager.log"
        self.print_output_file = self.base_dir / "co2_collector_print_output.txt"  # 采集器print输出记录
    ...
    def setup_logging(self):
        ...
        logging.info(f"CO2系统管理器日志已初始化: {self.log_file}")
        return logger
    ...
    def start_collector(self):
        ...
        try:
            ...
            with open(self.print_output_file, 'w', encoding='utf-8') as output_file:
                output_file.write(f"# CO2采集器启动 @ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            ...
            if process.poll() is None:  # 进程仍在运行
                success_msg = f"✅ CO2数据采集器启动成功 (PID: {process.pid})"
                print(success_msg)
                logging.info(f"采集器启动成功，进程PID: {process.pid}")
                return True
            else:
                # 进程已退出，读取输出文件查看错误信息
                error_msg = "❌ CO2数据采集器启动失败:"
                print(error_msg)
            ...
            error_msg = f"❌ 启动CO2数据采集器时出错: {e}"
            print(error_msg)
            logging.error(f"启动采集器异常: {e}")
            return False
    
    def cleanup_co2_files(self):
        """清理CO2系统相关文件"""
        print("🧹 清理CO2系统文件...")
        cleaned_count = 0
    ...
    def stop_collector(self):
        ...
        if not self.is_running():
            ...
            try:
                ...
                success_msg = "✅ CO2数据采集器已停止"
                print(success_msg)
                logging.info("采集器已成功停止")
                stopped = True
                
            except Exception as e:
                error_msg = f"❌ 停止CO2数据采集器时出错: {e}"
                print(error_msg)
                logging.error(f"停止采集器异常: {e}")
        
        # 无论采集器是否在运行，都清理CO2系统文件
        self.cleanup_co2_files()
        return stopped
                ...
                try:
                    ...
                    with open(latest_csv, 'r') as f:
                        ...
                        if rows:
                            ...
                            co2 = last_row.get('co2', 'N/A')
                            print(f"📊 最新数据: CO2={co2} ppm, 时间={timestamp}")
                except Exception as e:
                    print(f"❌ 读取数据文件出错: {e}")
            else:
    ...
    def view_live_data(self):
        ...
        try:
            while True:
                # 读取最新的CSV数据
                csv_file = self.logs_path / "co2_data.csv"
                if csv_file.exists():
                    try:
                        import csv
                        with open(csv_file, 'r') as f:
                            ...
                            if rows:
                                ...
                                timestamp = last_row.get('timestamp', 'N/A')
                                co2 = last_row.get('co2', 'N/A')
                                print(f"{time.strftime('%H:%M:%S')} 🟢 CO2={co2} ppm, 时间={timestamp}")
                            else:
