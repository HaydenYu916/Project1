...
# 创建全局读取器实例
_csv_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'logs', 'co2_data.csv')
_reader = CO2DataReader(_csv_path)

def get_current_co2(max_age_seconds=30):
    """
    获取当前CO2值（最常用的接口）
...
def get_co2_data(max_age_seconds=30):
    """
    获取详细的CO2数据
    
    Returns:
        dict: 包含value, timestamp, age_seconds, is_stale的完整数据
        或 None 如果无数据
    """
    return _reader.get_latest_value(max_age_seconds)

def get_co2_average(minutes=5):
    """
    获取指定时间内的平均CO2值
    ...
    Returns:
        float: 平均CO2值，如果无数据则返回None
    """
    return _reader.get_average(minutes)

def get_recent_co2(count=10):
    """
    获取最近的CO2读数列表
    ...
    Returns:
        list: 包含timestamp和value的字典列表
    """
    return _reader.get_recent_values(count)

__all__ = ['get_current_co2', 'get_co2_data', 'get_co2_average', 'get_recent_co2']